...
layui.use(['jquery', 'larryElem', 'layer', 'common', 'form', 'larryMenu', 'larryTab'], function () {
    ...
    $('#logout').on('click', function () {
        //var url = '/Backstage/Login/UserLoginOut';
        //console.log(url);
        //common.logOut('退出登陆提示！', '你真的确定要退出系统吗？', url);

        layer.confirm('你真的确定要退出系统吗？', { icon: 3, title: '退出' },
            function (index) {
                $.post("/Backstage/Login/UserLoginOut", function (data) {
                    window.location.href = "/Backstage/Login/Login";
                }, "json");
            });
    });
